...
from sales_predictor_light import SalesPredictorLight
...
def main():
    ...
    if not os.path.exists(data_file):
        ...
        generator.generate_all_data(years=1)
    else:
        print("加载已有数据...")
    ...
    print("=" * 60)
    print("步骤1: 训练销量预测模型 (RandomForest)")
    print("=" * 60)
    print()
    
    predictor = SalesPredictorLight()
    model_file = os.path.join("models", "sales_predictor_light.pkl")
    if os.path.exists(model_file):
        print("加载已有模型...")
        predictor.load_models()
        print(f"已加载 {len(predictor.models)} 个预测模型")
    else:
        print("训练销量预测模型中 (使用 RandomForest)...")
    ...
    if not waste_report.empty:
        ...
        for _, row in top_waste.iterrows():
            ing = data_storage.ingredients.get(row['ingredient_id'])
            unit = ing.unit if ing else ''
            print(f"  {row['store_name']} - {row['ingredient_name']}: "
                  f"{row['waste_rate']:.1%} "
                  f"(报废 {row['wasted_quantity']:.0f}{unit})")
    print()
    ...
    print("通过调用 update_model_with_new_data() 方法")
    ...
    print("核心功能总结:")
    print("  ✓ 9家门店销售数据采集与存储")
    print("  ✓ RandomForest时序预测模型 (气温、天气、节假日等特征)")
    ...
    print("  ✓ 每周模型迭代更新机制")
    print()
    print("注: 完整版本使用 XGBoost 模型，可获得更高预测精度")
    print()
